...
describe("api v1 export guards", () => {
  ...
  describe("handleRequest", () => {
    it("throws TypeError when request is null", async () => {
      await expect(handleRequest(null as unknown as Request)).rejects.toThrow(
        TypeError,
      );
    });

    it("throws TypeError when request is undefined", async () => {
      await expect(
        handleRequest(undefined as unknown as Request),
