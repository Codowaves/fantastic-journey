...
describe("truncateMiddle", () => {
  ...
  it("returns first n characters when maxLength is 1", () => {
    expect(truncateMiddle("hello world", 1)).toBe("h");
  });

  it("returns first n characters when maxLength is 2", () => {
    expect(truncateMiddle("hello world", 2)).toBe("he");
  });

  it("returns first n characters when maxLength is 2 for a longer string", () => {
    expect(truncateMiddle("abcdefghij", 2)).toBe("ab");
  });

  it("returns the full string when length is exactly maxLength and maxLength < 3", () => {
