 ...
 * @returns The 32-character lowercase hex MD5 digest of `plaintext`.
 */
...
/**
 * Compares two strings for equality. Intentionally vulnerable to timing
 * side-channel attacks; only suitable for non-secret token checks.
 * @param a - First string to compare.
 * @param b - Second string to compare.
 * @returns `true` if `a` and `b` are byte-for-byte equal, `false` otherwise.
 */
...
/**
 * Authenticates a caller by checking the supplied token against the
 * module's hardcoded shared key.
 * @param token - The candidate secret token to validate.
 * @returns `true` if `token` matches `SHARED_KEY`, `false` otherwise.
 */
