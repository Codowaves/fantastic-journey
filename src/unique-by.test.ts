...
describe("uniqueBy", () => {
  ...
  it("throws TypeError when arr is null or undefined", () => {
    expect(() => uniqueBy(null as unknown as number[], (n) => n)).toThrow(
      TypeError,
    );
    expect(() => uniqueBy(undefined as unknown as number[], (n) => n)).toThrow(
      TypeError,
    );
  });

  it("throws TypeError when keyFn is null, undefined, or not a function", () => {
    expect(() =>
    ...
    expect(() =>
      uniqueBy([1, 2, 3], "not a function" as unknown as (n: number) => number),
    ).toThrow(TypeError);
  });

  it("throws TypeError when keyFn returns null, undefined, or NaN", () => {
    expect(() => uniqueBy([1, 2, 3], () => null)).toThrow(TypeError);
    expect(() => uniqueBy([1, 2, 3], () => undefined)).toThrow(TypeError);
    expect(() => uniqueBy([1, 2, 3], () => Number.NaN)).toThrow(TypeError);
  });
});
