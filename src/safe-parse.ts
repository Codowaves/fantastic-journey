 ...
 *   parse failure.
 */
export function safeParseInt(input: string): Result<number, Error> {
 ...
 *   parse failure.
 */
export function safeParseFloat(input: string): Result<number, Error> {
  if (typeof input !== "string") {
    return err(
 ...
 *   parse error.
 */
export function safeParseJson<T = unknown>(input: string): Result<T, Error> {
  try {
